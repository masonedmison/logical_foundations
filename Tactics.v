...
Search list inside Poly.

(** **** Exercise: 3 stars, standard (injection_ex3) *)
(* feels suspicious that I need to use `rewrite in` like this... *)
...
Proof.
  intros f b.
  destruct b eqn: EqA.
  - destruct (f true) eqn: EqB.
    + rewrite EqB. rewrite EqB. reflexivity.
    + destruct (f false) eqn: EqC.
      * rewrite EqB. reflexivity.
      * rewrite EqC. reflexivity.
  - destruct (f false) eqn: EqB.
    + destruct (f true) eqn: EqC.
      * rewrite EqC. reflexivity.
      * rewrite EqB. reflexivity.
    + rewrite EqB. rewrite EqB. reflexivity.
Qed.
...
(* Lemma list_fun_application : forall (X : Type) (f1 f2: list X -> bool) (l1 l2 : list X),
  f1 l1 = f2 l2 -> f1 (tl l1) = f2(tl l2).  *)
