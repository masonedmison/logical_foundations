...
Proof.
  ...
  unfold t_update. apply String.eqb_neq in H.
...
Lemma first_if : forall (A: Type) (x y: string) (r1 r2 r3: A),
  (if (x =? y)%string then r1 else if (x =? y)%string then r2 else r3) = (if (x =? y)%string then r1 else r3)
.
Proof.
  intros A x y r1 r2 r3.
...
Proof.
  ...
  - rewrite <- e. rewrite <- String.eqb_neq in H. 
    rewrite String.eqb_sym. rewrite H. reflexivity.
