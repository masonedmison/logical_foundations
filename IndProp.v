...
(* FILL IN HERE
    [] *) 
Inductive clos_trans2 {X: Type} (R: X -> X -> Prop) : X->X->Prop :=
  | t_step (x y : X) :
  ...
  | t_trans (x y z : X) :
      ...
      clos_trans R x z
  | t_sym(x y : X) :
      clos_trans R x y -> close_trans R y x
  | t_refl(x y : X) :
      x = y -> clos_trans R x x = clos_trans y y.
...
Proof.
  ...
  - intros m Hm.
    destruct Hm as [|m' Hm'].
    + rewrite <- plus_n_O. apply ev_SS. apply Hn'.
    + simpl. apply ev_SS. apply IHn. apply ev_SS. apply Hm'.
Qed.
...
Inductive total_relation : nat -> nat -> Prop :=
  | total_all (n m: nat) : total_relation n m.
...
Inductive empty_relation : nat -> nat -> Prop :=
  | empty_all (n m: nat) :  false = true -> empty_relation n m.

Theorem empty_relation_is_empty : forall n m, ~ empty_relation n m.
  Proof.
    intros n m H. inversion H as []. discriminate H0.
Qed.
(** [] *)
...
Proof.
  ...
  induction n as [|n' IH].
  - reflexivity.
  - rewrite IH. apply le_S. reflexivity.
Qed.
...
Proof.
  intros n m H.
  induction H as [|n' IH HEq].
  - reflexivity.
  - rewrite HEq. apply le_S. reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - rewrite <- HH. apply le_S. reflexivity.
Qed.
...
Proof.
  ...
  - destruct IHle as [IHLeft IHRight]. split.
    + apply le_S. apply IHLeft.
    + apply le_S. apply IHRight.
Qed.

Lemma succ_n_le : forall n m,
  S n <= m -> n <= m.
Proof.
  intros n m.
...
Admitted.
  (* - destruct H.
    + destruct IH.
      * rewrite plus_Sn_m. apply le_S. reflexivity.
      * left.      *)
    (* destruct IH.
    + rewrite plus_Sn_m in H. rewrite <- H. apply le_S. reflexivity.
    + apply plus_le in H. destruct H as [HL HR].
    left. rewrite <- H0. rewrite HL. (* apply (le_plus_trans n' p q) in H0.   *)
    apply succ_n_le in HL. *)

    (* rewrite plus_Sn_m in H.
    apply succ_n_le in H.
    apply IH in H.
...
Proof.
  intros n m p H.
  induction p as [|p' IH].
  - rewrite plus_O_n. rewrite plus_O_n. apply H.
...
Proof.
  ...
  - rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. apply n_le_m__Sn_le_Sm. apply IH.
Qed.
...
Proof.
  intros n m H.
  unfold lt in H. apply succ_n_le in H. apply H.
Qed.
...
Proof.
  ...
  - unfold lt in H. unfold lt.
    rewrite <- H. apply n_le_m__Sn_le_Sm. rewrite add_comm. apply le_plus_l.
Qed.
(** [] *)
...
(** - Which of the following propositions are provable?
      - [R 1 1 2]
        1:
          apply c2 ==> (R (S 0) 1 (S 1)) ==> R 0 1 1
          apply c3 ==> R 0 (S 0) (S 0) ==> R 0 0 0
          apply c1
          Qed.

      - [R 2 2 6]
        1:
          apply c2 ==> R (S 1) 2 (S 5) ==> R 1 2 5
          apply c3 ==> R 1 (S 1) (S 4) ==> R 1 1 4
          apply c3 ==> R 1 (S 0) (S 3) ==> R 1 0 3
          S1:
            apply c2 ==> R (S 0) 0 S (2) ==> R 0 0 2
            Abort.
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      No, the swapping of the m and n arguments does not help since we have both of
      the c2 and c3 variants that already account for n or m and o.

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer. 
      No, the Relation is only provable when the first two natural numbers added equal the third,
      e.g. 1 1 2 is approvable, 1 1 3 is not. For all n m o: nat, n + m < 0 -> S n + Sm < S (S o).
      see inc_n_m_le_inc_inc_o for the proof. *)

(* FILL IN HERE *)

Theorem inc_n_m_le_inc_inc_o: forall n m o: nat,
  n + m < o -> S n + S m < S (S o).
Proof.
  intros n m o H.
  unfold lt in H. unfold lt.
  apply n_le_m__Sn_le_Sm.
  simpl. apply n_le_m__Sn_le_Sm. rewrite <- plus_n_Sm. apply H.
Qed.
...
Search list in Logic.


Inductive subseq : list nat -> list nat -> Prop :=
  | subseq_done (_ l2 : list nat) : subseq nil l2 
  | subseq_matched_h (lh: nat) l1 (rh: nat) l2 : lh = rh /\ subseq l1 l2 -> subseq (lh :: l1) (rh :: l2) 
  | subseq_non_matched l1 (rh: nat) l2 : subseq l1 l2 -> subseq l1 (rh :: l2)
.

Example subseq_test1 : subseq [1;2;3] [1;1;1;2;2;3].
Proof.
  apply subseq_matched_h.
  split. reflexivity.
  apply subseq_non_matched. apply subseq_non_matched.
  apply subseq_matched_h.
  split. reflexivity.
  apply subseq_non_matched.
  apply subseq_matched_h.
  split. reflexivity.
  apply (subseq_done [] []).
Qed.
...
Example re_not_empty_test1 : re_not_empty (App (Char 1 ) (Char 2)) = true.
Proof. reflexivity. Qed.

Example re_not_empty_test2 : re_not_empty (App (Char 1 ) EmptySet) = false.
  Proof. reflexivity. Qed.
...
Proof.
  intros T re. 
  split.
  - intros H. destruct H as [s HH].
    induction HH.
    + reflexivity.
    + reflexivity.
    + simpl. rewrite IHHH1. rewrite IHHH2. reflexivity.
    + simpl. rewrite IHHH. reflexivity.
    + simpl. rewrite IHHH. rewrite orb_true_iff. right. reflexivity.
    + reflexivity.
    + reflexivity.
  - intros H.
    induction re.
    ...
    + simpl in H. rewrite andb_true_iff in H. destruct H as [HL HR].
      apply IHre1 in HL. destruct HL as [s' HL'].  apply IHre2 in HR. destruct HR as [s'' HR'].
      exists (s' ++ s'').
      apply MApp.
      * apply HL'.
      * apply HR'.
...
Proof.
  ...
  induction Hmatch.
    ...
    - exists []. split.
      + reflexivity.
      + simpl. intros s' H. destruct H.
    - 
      rewrite <- Heqre' in IHHmatch1.  injection Heqre'. intros inj.
      apply IHHmatch2 in Heqre'. destruct Heqre' as [ss [Hs2match HInmatch]].
      exists ([s1] ++ ss). split.
      + rewrite Hs2match. reflexivity.
      + simpl. intros s H. destruct H as [H' | H'].
        * rewrite H' in Hmatch1. rewrite inj in Hmatch1. apply Hmatch1.
        * apply HInmatch. apply H'.
Qed.
...
Proof.
  ...
  - simpl. intros H. apply le_S_n in H. inversion H.
  - simpl. intros H. rewrite app_length in H.
    apply add_le_cases in H. destruct H as [H' | H'].
    + apply IH1 in H'. destruct H' as [s5 [s4 [s3 [HLmatch [HRLmatch HRRmatch]]]]].
    rewrite HLmatch.
    exists s5. exists s4. exists (s3 ++ s2). split.
      * rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      * split. 
        ** apply HRLmatch.
        ** intros m.
        assert (HAssoc: s5 ++ napp m s4 ++ s3 ++ s2 = (s5 ++ napp m s4 ++ s3) ++ s2).
        { rewrite <- app_assoc. rewrite <- app_assoc. reflexivity. }
        rewrite HAssoc. apply MApp.
          *** apply HRRmatch.
          *** apply Hmatch2.
    + apply IH2 in H'. destruct H' as [s3' [s2' [s1' [HLmatch [HRLmatch HRRmatch]]]]].
      * rewrite HLmatch. exists (s1 ++ s3').  exists s2'. exists s1'. split.
        ** rewrite <- app_assoc. reflexivity.
        ** split.
          *** apply HRLmatch.
          *** intros m. rewrite <- app_assoc. apply (MApp s1).
            **** apply Hmatch1.
            **** apply HRRmatch.
  - intros H. simpl in H. apply plus_le in H. destruct H as [H' _].
    apply IH in H'.  destruct H' as [s5 [s4 [s3 [HLmatch [HRLmatch HRRmatch]]]]].
    rewrite HLmatch. exists s5. exists s4. exists s3. split.
    * reflexivity. 
    * split.
      ** apply HRLmatch.
      ** intros m. apply MUnionL. apply HRRmatch.
  - intros H. simpl in H. apply plus_le in H. destruct H as [_ H'].
    apply IH  in H'. destruct H' as [s5 [s4 [s3 [HLmatch [HRLmatch HRRmatch]]]]].
    rewrite HLmatch. exists s5. exists s4. exists s3. split.
    * reflexivity.
    * split.
      ** apply HRLmatch.
      ** intros m. apply MUnionR. apply HRRmatch.
  - intros H. simpl in H.  inversion H.
    apply pumping_constant_0_false in H1. destruct H1.
  - 
    intros H.
    destruct s2 eqn: EqA.
    + rewrite app_nil_r in H. simpl in H.
      apply IH1 in H. destruct H as [s5 [s4 [s3 [HLmatch [HRLmatch HRRmatch]]]]].
      rewrite HLmatch. exists s5. exists s4. exists s3. split.
      * rewrite <- app_assoc. rewrite <- app_assoc. rewrite app_nil_r. reflexivity.
      * split. 
        ** apply HRLmatch.
        ** intros m. rewrite <- (app_nil_r T (s5 ++ napp m s4 ++ s3)).
          apply (MStarApp(s5 ++ napp m s4 ++ s3) [] re).
          *** apply HRRmatch.
          *** apply MStar0.
    + rewrite <- EqA. exists s1. exists s2. exists [].
      split.
      * rewrite EqA. rewrite app_nil_r. reflexivity.
      * split.
        ** unfold not. intros HH. rewrite HH in EqA. discriminate EqA.
        ** intros m. rewrite <- EqA in Hmatch2.
          rewrite app_nil_r. apply MStarApp.
          *** apply Hmatch1.
          *** induction m.
            **** simpl. apply MStar0.
...
Proof.
  ...
  - induction H.
    ...
    + intros HH. apply ex_falso_quodlibet. apply H. apply HH.
  - intros HH. induction H.
    + apply H.
    + discriminate HH.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. simpl in Hcount. destruct (eqbP n m) as [HEq | HEq].
    + simpl in Hcount. discriminate Hcount.
    + simpl in H. destruct H as [H' | H'].
      * apply HEq. rewrite H'. reflexivity.
      * apply IHl'. 
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | ns_base0 : nostutter []
  | ns_base1 (x: X) : nostutter (x :: nil)
  | ns_ind (x y : X) (rem: list X) :  x <> y -> nostutter (y :: rem) -> nostutter (x :: y :: rem)
.
