...
Proof.
  ...
  -
    induction l as [|a' l' IHl'].
    +
      simpl. intros [x [ H H']]. destruct H'.
    + 
      simpl.  intros [x [H [H' | H']]].
      * rewrite <- H' in H. left. apply H.
      * 
        right. 
        apply IHl'.
        exists x. 
...
Proof.
  ...
  -
    ...
    + intros H x H'. simpl in H'. destruct H'.
  -
    split.
    + 
      simpl. 
...
Check add_comm : forall x y : nat, x + y = y + x.
...
Lemma append_list_rev_append: forall X (l1 l2: list X),
...
Proof.
  ...
  -
    ...
    + 
      simpl. 
      rewrite IHl1.
      assert(H: rev_append l1' [a'] = rev_append l1' [ ] ++ [a']).
...
Proof.
  ...
  - 
    ...
    simpl.
    rewrite <- IHl.
    apply (append_list_rev_append X).
Qed.
(** [] *)
...
Proof.
  intros n.
  induction n as [|n' [k H]].
  - exists 0. reflexivity.
  - 
    rewrite even_S.
    destruct (even (n')) eqn: EqA.
    +
      simpl.
      exists k.
      rewrite H.
...
Proof.
  ...
  - 
    ...
    intros H H'.
    apply eqb_true in H'. apply H in H'. destruct H'.
Qed.
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | nil, nil => true
  | lh :: lt, rh :: rt => 
    (eqb lh rh) && eqb_list eqb lt rt
  | _, _ => false
  end.

Example eqb_list_test1: eqb_list eqb [1;2;3] [1;2;3] = true.
Proof.
  simpl. reflexivity. Qed.

Example eqb_list_test2: eqb_list eqb [1;2;3] [1;2;3;4] = false.
Proof.
  simpl. reflexivity. Qed.
  ...
  - generalize dependent l2. induction l1 as [|a l1' IHl].
    ...
    + destruct l2 as [|a' l2'].
      ...
      * intros H'. simpl in H'. rewrite andb_true_iff in H'. 
        destruct H' as [H' H'']. rewrite H in H'. 
        rewrite H'. f_equal. apply IHl. apply H''.
  - generalize dependent l2. induction l1 as [|a l1' IHl].
    + destruct l2 as [|a' l2'].
      * intros []. reflexivity.
      * intros []. reflexivity.
    + destruct l2 as [|a' l2'].
      * intros H'. discriminate H'.
      * intros H'. injection H' as H' H''. simpl. rewrite andb_true_iff.
        split.
        rewrite H'. apply H. reflexivity.
        apply IHl. apply H''.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [|a l' IHl].
    ...
    + simpl. intros [H H']. rewrite andb_true_iff.
      split.
      apply H.
      apply IHl. apply H'.
Qed.
